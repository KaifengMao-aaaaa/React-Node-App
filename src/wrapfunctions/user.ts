...
export async function userRegister(name: string, email: string, password:string, verficationCode: string) {
    ...
    try {
        await insert('users', ['name', 'email', 'password', 'ID', 'time', 'role'],[name, email, hashedPassword, staffId, new Date(), permissions.NONE])
    } catch(e) {
        throw createHttpError(403, '插入数据库失败')
    }
    if (email === process.env.CREATOREMAIL) {
        await update('users', ['role'], [permissions.CREATOR], ['ID'], [staffId])
    }
...
export async function userPermissionModify(id: number,staffId: number, toPermission) {
    ...
    if (operator[0].role === permissions.NONE || operator[0].role === permissions.WORKER || toPermission < operator[0].role) {
